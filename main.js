...
function cultureForProvince(prov, era) {
  return era.cultures[prov.region] || era.cultures[prov.id] || era.cultures['ameriques_sud'] || 'Culture libre';
}

function updateCountryOptions(era) {
  const cultures = new Set();
  PROVINCES.forEach((prov) => cultures.add(cultureForProvince(prov, era)));
...
function createWorld() {
  ...
  PROVINCES.forEach((prov) => {
    const culture = cultureForProvince(prov, era);
...
function draw() {
  ...
  PROVINCES.forEach((prov) => {
    ...
    ctx.strokeStyle = highlighted ? 'rgba(255, 255, 255, 0.45)' : 'rgba(255,255,255,0.1)';
